...
6. **获取配置**: 节点底层的防封锁参数配置完成后，终端将打印出一个 `vless://` 链接。

---

## 📱 客户端配置指南

获取到 `vless://` 链接后，请根据您的操作系统下载并配置对应的代理客户端：

### Windows
- **推荐客户端**: **v2rayN** 或 **Clash Verge (Meta 内核)**
- **操作方式**: 
  1. 复制终端中生成的 `vless://...` 完整链接。
  2. 打开 v2rayN，在系统托盘右键图标或主界面菜单中选择 **“从剪贴板导入批量 URL” (Import from Clipboard)**。
  3. 选中新导入的节点，按 `Enter` 键设为活动节点即可使用。

### macOS
- **推荐客户端**: **V2RayXS** 或 **Clash Verge (Meta 内核)**
- **操作方式**: 
  1. 复制生成的链接。
  2. 打开 V2RayXS，选择 **“Import from Clipboard”**。
  3. 配置系统代理即可。

### iOS (iPhone/iPad)
- **推荐客户端**: **Shadowrocket (小火箭)** 或 **V2ray Tun**
- **操作方式**: 
  1. 将链接发送到手机并复制。
  2. 打开 Shadowrocket，软件通常会自动识别剪贴板并提示导入。如果没有提示，点击右上角 `+` 号，类型选择 `Subscribe` (或直接选择 `Vless` 手动粘贴链接)，点击右上角保存。
...
- **操作方式**: 
  ...
  3. 选择 **“从剪贴板导入”**，然后点击右下角的 `V` 形图标启动连接。
